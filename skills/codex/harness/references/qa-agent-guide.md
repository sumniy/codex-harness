...
### 3-1. explorer 타입이 아닌 default 타입을 사용하라

QA 에이전트가 `explorer` 타입이면 읽기만 가능하다. 하지만 효과적인 QA는:
- Grep으로 패턴 검색 (모든 `NextResponse.json()` 추출)
- 스크립트 실행으로 자동 대조 (API shape vs 훅 타입)
- 필요 시 수정까지 가능

**권장**: `default` 타입으로 설정하되, 에이전트 정의에서 "검증 → 리포트 → 수정 요청" 프로토콜을 명시.
...
빌드 하네스의 QA 에이전트에 포함할 핵심 섹션. `.codex/agents/qa-inspector.toml`에 저장한다.
...
- 검증 결과를 `.codex/reports/qa-report.md` 파일에 기록한다
- 발견 즉시 구체적 수정 요청을 리포트에 포함 (파일:라인 + 수정 방법)
- 경계면 이슈는 관련 파일 양쪽 모두를 리포트에 명시
- 리포트에 통과/실패/미검증 항목을 명확히 구분하여 기재
"""
```
